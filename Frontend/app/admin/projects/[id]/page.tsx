...
export default function AdminProjectDetailPage() {
  ...
  const fetchProjectMessages = async () => {
    ...
    try {
      // Use the new admin endpoint to get project messages
      const url = `${
      ...
      if (data?.success) {
        ...
        // Filter files to only show messages with the same projectId
        const filteredFiles = allMessages.filter(
          (msg: any) => String(msg.projectId) === String(projectId)
        );
        ...
        <div className="flex gap-4">
          ...
          {isServiceRequest && project.proposalsCount > 0 && (
            <Badge className="bg-blue-100 text-blue-800">
              ...
              {project.proposalsCount === 1 ? "proposal" : "proposals"}
            </Badge>
          )}
        </div>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        ...
                        {project.provider?.providerProfile?.rating !==
                          undefined &&
                          project.provider.providerProfile.rating !== null && (
                            <div className="flex items-center gap-2">
                    ...
                    {project.proposals.map((proposal: any) => (
                      <div key={proposal.id} className="border rounded-lg p-4">
                        ...
                        {proposal.attachmentUrls &&
                          proposal.attachmentUrls.length > 0 && (
                            <div className="mt-4">
                              <p className="text-sm font-medium text-gray-700 mb-2">
                                Attachments
                              </p>
                              <div className="space-y-2">
                                {proposal.attachmentUrls.map(
                ...
                {(() => {
                  ...
                  if (isServiceRequest && project.proposals) {
                    project.proposals.forEach((proposal: any) => {
                      if (
                        proposal.attachmentUrls &&
                        Array.isArray(proposal.attachmentUrls)
                      ) {
                  ...
                  {projectMessages.length > 0 && (
                    <div className="flex justify-center">
                      <p className="text-sm text-gray-500">
                        ...
                        {project.customer?.name || "Client"} and{" "}
