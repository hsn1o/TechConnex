...
import { Heart, Briefcase, Search, Sparkles, ChevronRight, AlertTriangle, Star, MapPin, MessageSquare, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    const fetchRecommendedProviders = async () => {
      try {
        ...
        setErrorRecommended(
          err instanceof Error ? err.message : "Failed to fetch recommended providers"
        );
        setRecommendedProviders([]);
      } finally {
      ...
      <Tabs defaultValue="all" className="space-y-4 sm:space-y-6">
        ...
        <TabsContent value="all" className="space-y-4 sm:space-y-6">
          ...
          {loading ? (
            <div className="text-center py-8 sm:py-12">
              ...
              <p className="text-sm sm:text-base text-gray-600">Loading providers...</p>
            </div>
          ) : (
        ...
        <TabsContent value="recommended" className="space-y-4 sm:space-y-6">
          ...
          {loadingRecommended ? (
            <div className="text-center py-8 sm:py-12">
              <Loader2 className="w-8 h-8 sm:w-12 sm:h-12 text-gray-400 mx-auto mb-4 animate-spin" />
              <p className="text-sm sm:text-base text-gray-600">Loading recommended providers...</p>
            </div>
          ) : errorRecommended ? (
            <Card>
                  ...
                  >
                    ...
                    <CardContent className="p-0">
                            ...
                            <div className="p-3 sm:p-4 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-lg border-2 border-blue-200 shadow-md">
                              ...
                              <div className="text-xs sm:text-sm text-blue-800 space-y-1.5 sm:space-y-2">
                                {provider.aiExplanation.split('\n').filter((line: string) => line.trim()).map((line: string, index: number) => {
                                  ...
                                  return cleanLine ? (
                                    <div key={index} className={`flex items-start gap-2 sm:gap-3 ${isWarning ? 'bg-red-50 p-2 rounded border border-red-200' : ''}`}>
                                      <span className={`mt-0.5 font-bold flex-shrink-0 ${isWarning ? 'text-red-600' : 'text-blue-600'}`}>•</span>
                                      <span className={`leading-relaxed break-words ${isWarning ? 'text-red-800 font-medium' : ''}`}>{cleanLine}</span>
                                    </div>
                                  ) : null;
                                })}
                              </div>
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 mt-3 sm:mt-4 pt-3 border-t border-gray-200 group-hover:border-blue-200 transition-colors">
                        ...
                        <Link 
                          href={`/customer/messages?userId=${provider.id}&name=${encodeURIComponent(provider.name)}&avatar=${encodeURIComponent(provider.avatar || "")}`}
                          className="flex-1"
                        >
                          <Button
                            size="sm"
